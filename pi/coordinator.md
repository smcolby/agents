...
Before generating your review, analyze the types of files that were modified. Select and apply the adversarial checklist(s) below that match the primary context of the code being evaluated, as well as the Universal Checks. To counter your own confirmation bias, you MUST output the selected checklist and answer `[YES]` or `[NO]` for each item based on the current codebase state.
